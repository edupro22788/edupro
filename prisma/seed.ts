...
const FACULTIES: { name: string; majors: string[] }[] = [
  {
    ...
    majors: ['اللغة العربية وآدابها', 'اللغة الفرنسية', 'اللغة الإنجليزية', 'اللغة الأمازيغية'],
  },
  {
    name: 'كلية العلوم الإنسانية والاجتماعية',
    majors: ['علم النفس', 'علم الاجتماع', 'التاريخ', 'الفلسفة', 'علوم الإعلام والاتصال'],
  },
  {
    name: 'كلية الحقوق والعلوم السياسية',
    majors: ['القانون', 'العلوم السياسية', 'العلاقات الدولية'],
  },
  {
    name: 'كلية العلوم الاقتصادية والتجارية وعلوم التسيير',
    majors: ['العلوم الاقتصادية', 'علوم التسيير', 'العلوم المالية والمحاسبية'],
  },
  {
    name: 'كلية العلوم الدقيقة وعلوم الطبيعة والحياة',
    majors: ['الرياضيات', 'الفيزياء', 'الكيمياء', 'علوم الطبيعة والحياة', 'البيولوجيا'],
  },
  {
    name: 'كلية علوم التكنولوجيا',
    majors: ['الإعلام الآلي', 'الإلكترونيك', 'الهندسة المدنية', 'الهندسة الكهربائية', 'الطاقات المتجددة'],
  },
];
...
async function main() {
  ...
  for (let i = 0; i < WILAYAS.length; i++) {
    ...
    if (!existing) {
      const university = await prisma.university.create({ data: { name: uniName, stateId: state.id } });
      // الكليات والتخصصات والمستويات
