...
const INLINE_TYPES = ['application/pdf', 'image/png', 'image/jpeg', 'image/webp', 'image/gif', 'text/plain'];
...
export async function GET(_req: NextRequest, ctx: Ctx) {
  ...
  // لا يُسمح إلا للأعضاء بالوصول، والمنشور فقط، أو صاحبه، أو مشرف الفوج
  const allowed =
  ...
  response.headers.set('X-Edupro-Access', 'authorized');
  return response;
}